...
在完成需求分析后，本章进一步给出系统设计方案。基于树莓派的人脸识别考勤系统并非单一的人脸识别模型调用，而是由人员准入、注册采集、资料审核、终端识别、活体检测、考勤写库和远程管理等环节共同构成的应用系统。系统设计需要同时回答三个问题：系统功能如何划分，运行结构如何组织，关键业务如何在数据和接口之间闭环。系统的核心业务链路可以概括为：管理员预置成员名单，用户完成首次注册和动作挑战，系统对注册照进行质量校验，管理员审核通过后该人脸档案进入正式识别库，树莓派终端在现场完成实时识别和考勤写入，Windows管理端通过Tailscale网络查询设备、成员、人脸档案和考勤数据。

\section{系统总体设计}

系统架构设计首先要明确系统面对的使用对象和功能边界。本文系统面向三类对象：第一类是普通用户，主要完成首次注册、登录、查看人脸资料状态和提交注册照；第二类是现场终端，树莓派以无头模式运行后端服务，终端页面可由同一网络内的浏览器访问，主要提供摄像头视频流、识别状态提示和自动签到；第三类是管理员，主要通过Windows管理端完成成员名单维护、人脸档案审核、考勤查询和设备状态巡检。三类对象对系统的要求不同：普通用户关注流程是否清晰、失败原因是否可理解；现场终端关注识别是否实时、提示是否简洁；管理员关注数据是否可追溯、设备是否可远程维护。为避免不同入口互相干扰，系统在功能上被划分为用户门户、注册审核、终端识别、数据管理和远程管理五部分，如图\ref{fig:function_structure}所示。
...
系统的实际部署方式如图\ref{fig:deployment_architecture}所示。树莓派端是系统的核心运行节点，以无头模式连接USB摄像头并运行FastAPI服务，同时保存SQLite数据库、注册照和考勤快照。同一网络内的浏览器访问树莓派根路径进入终端识别页面；普通用户从非本地网络访问时进入用户门户；Windows管理端通过Tailscale网络调用树莓派暴露的管理员API。这样部署的好处是现场签到不依赖外部服务器，管理员又可以在不直接暴露公网端口的情况下远程完成审核和查询。
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[device] (terminal) at (-5,-2.0) {终端浏览器\\识别页面};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        flowbox/.style={rectangle, draw=black, rounded corners=3pt, align=center, minimum width=3.25cm, minimum height=0.78cm, fill=gray!7},
        decisionbox/.style={diamond, draw=black, aspect=2.15, align=center, inner sep=1pt, fill=gray!5},
        datastore/.style={cylinder, shape border rotate=90, draw=black, aspect=0.25, align=center, minimum width=1.75cm, minimum height=0.82cm, fill=gray!5},
        arrow/.style={->, >=stealth, thick}
    ]
        \node[flowbox] (frame) {摄像头实时帧};
        \node[flowbox, below=of frame] (detect) {YuNet人脸检测};
        \node[decisionbox, below=of detect] (facegate) {单人且\\尺寸合格};
        \node[flowbox, below=of facegate] (live) {活体检测与重放风险评估};
        \node[decisionbox, below=of live] (livegate) {活体可信};
        \node[flowbox, below=of livegate] (match) {SFace特征提取与身份匹配};
        \node[decisionbox, below=of match] (matchgate) {身份可信};
        \node[decisionbox, below=of matchgate] (rulegate) {考勤规则\\通过};
        \node[flowbox, below=of rulegate] (write) {保存快照并写入考勤记录};
        \node[flowbox, right=2.4cm of livegate] (reject) {返回提示\\等待下一帧};
        \node[datastore, left=2.3cm of match] (db) {审核通过\\人脸库};

        \draw[arrow] (frame) -- (detect);
        \draw[arrow] (detect) -- (facegate);
        \draw[arrow] (facegate) -- node[right]{是} (live);
        \draw[arrow] (live) -- (livegate);
        \draw[arrow] (livegate) -- node[right]{是} (match);
        \draw[arrow] (match) -- (matchgate);
        \draw[arrow] (matchgate) -- node[right]{是} (rulegate);
        \draw[arrow] (rulegate) -- node[right]{是} (write);
        \draw[arrow] (facegate.east) -| node[above]{否} (reject.north);
        \draw[arrow] (livegate.east) -- node[above]{否} (reject.west);
        \draw[arrow] (matchgate.east) -| node[below]{否} (reject.south);
        \draw[arrow] (rulegate.east) -| node[below]{否} (reject.south);
        \draw[arrow] (write.east) -| (reject.south);
        \draw[arrow, dashed] (db) -- node[above]{提供特征} (match);
    \end{tikzpicture}}
    \caption{端侧识别与考勤写入流程图}
...
接口设计与数据库设计保持一致，按访问对象分为用户门户接口、动作挑战接口、终端识别接口和管理员接口。用户门户接口依赖浏览器Session，适合普通用户登录后查看和提交自己的资料；终端识别接口面向现场识别页面，主要提供视频流和识别状态；管理员接口面向Windows管理端，必须携带Token。接口分组如表\ref{tab:api_group_detail}所示。
